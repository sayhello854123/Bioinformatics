dir.create('cibersoft')
pFilter=0.05
rt=read.table("./cibersoft/CIBERSORTx_Job1_Results.txt",sep="\t",header=T,row.names=1,check.names=F)    #读取文件
data=rt[rt[,"P-value"]<pFilter,]
data=data[,1:(ncol(rt)-3)]

risk=read.table("./cox/tcgaRisk.txt",header=T,sep="\t",row.names=1,check.names=F)
rownames(risk) = substr(rownames(risk),1,12)
...
write.table(outTab,file="./cibersoft/TCGA/corStat.txt",sep="\t",row.names=F,quote=F)
fmsb=rbind(colMeans(dataHigh),colMeans(dataLow))
row.names(fmsb)=c("High","Low")
write.table(cbind(id=rownames(fmsb),fmsb),file="./cibersoft/TCGA/fmsbInput.txt",sep="\t",quote=F,row.names=F)

library(fmsb) 
data=read.table("./cibersoft/TCGA/fmsbInput.txt",header=T,sep="\t",row.names=1,check.names=F)   #读取输入文件
...
colors=c("red","blue")
#定义显著性
corStat=read.table("./cibersoft/TCGA/corStat.txt",header=T,sep="\t",row.names=1,check.names=F)
corStat=corStat[sortCellNames,]
colnames(data)=paste0(colnames(data),corStat$sig)

#输出结果
pdf(file="./cibersoft/TCGA/radar.pdf",height=8,width=8)
