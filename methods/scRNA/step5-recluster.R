...
##提取细胞子集
Cells.sub <- subset(scRNA@meta.data, celltype=="Epithelial_cells")
scRNAsub <- subset(scRNA, cells=row.names(Cells.sub))
'''
因为再聚类的细胞之间差异比较小，所以聚类函数FindClusters()
控制分辨率的参数建议调高到resolution = 0.9。
'''
##PCA降维
...
##细胞类型鉴定
library(SingleR)
refdata <- celldex::MonacoImmuneData()
